...
/* 10.2.16.3.x CryptGetPufId() */
/* Send a challenge and receive the response from the PUF */
LIB_EXPORT UINT16
CryptGetPufId(
		    UINT32           pufidSize,
		    BYTE            *buffer
		    )
{
    ...
    //declare strings we will use for the challenge message
    //will be used to create unique response message by using RO and FPGA
    //const char *msg1 = "SELA;HxA7;\r";
    //const char *msg2 = "SELB;HxDE;\r";
    //start;now; sends the two SEL(a/b) messages to get the response
    //const char *startMsg = "START;NOW;\r";
    
    //create the port for the USB device/circuit board with 
    serial_port_create("/dev/ttyUSB1");
    //write/send messages to the board
    serial_port_write("SELA;HxA7;\r");
    serial_port_write("SELB;HxDE;\r");
    serial_port_write("START;NOW;\r");
    
    
    //declare a input buffer, read in the response after 
    printf("about to read in buffer");
    //set a new input buffer to the passed buffer, read in the answer response
    char *input = *buffer;
    serial_port_read(input);
    //destroy serial port after use, similar to free() for malloc()
    serial_port_destroy();
    
    //create temp array to hold the hex value in response
    char numArray[2];
    
    //copy the hex values over to the temp array
    //hex values will be in same location in the answer string every time
    const char *hexNum1 = &input[7];
    const char *hexNum2 = &input[8];
    
   memcpy(numArray, hexNum1, 1);
   memcpy(numArray+1, hexNum2, 1);
   	
   	//convert the string value into a long hex, base 16 = hex 
   long int value = strtol(numArray, NULL, 16);
   if (value == 0){
   	printf("strtol failed");
   }
   //convert hex long into integer which can be returned
   int intValue = (int)value;
    // Place code to send challenge and receive response over serial port
	// Challenge and response are hex represented as character strings, return
	// value must be same hex value represented as integer
    buffer[0]=intValue; // replace hardwired value with PUF response
    //return DRBG_Generate((RAND_STATE *)&drbgDefault, buffer, pufidSize);
    return pufidSize+1;
